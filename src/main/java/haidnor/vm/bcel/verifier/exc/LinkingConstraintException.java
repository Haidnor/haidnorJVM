 ...
 */
package haidnor.vm.bcel.verifier.exc;

/**
 * Instances of this class are thrown by BCEL's class file verifier "JustIce" when a class file to verify does not pass
 * the verification pass 3 because of a violation of a constraint that is usually only verified at run-time (pass 4).
 * The Java Virtual Machine Specification, 2nd edition, states that certain constraints are usually verified at run-time
 * for performance reasons (the verification of those constraints requires loading in and recursively verifying
 * referenced classes) that conceptually belong to pass 3; to be precise, that conceptually belong to the data flow
